local function update()
   ...
   local current_config = nvchad.load_config()
   ...
   local function check_for_breaking_changes(current_head, remote_head)
      ...
      if new_commit_list == nil or #new_commit_list == 0 then
         ...
         echo {
            ...
            {
               "\n\nWould you like to reset NvChad to the remote's HEAD? Local changes will be lost! " .. "[y/N]",
               "WarningMsg",
            },
         }
      ...
      if #breaking_changes == 0 then
         print_progress_percentage(
            "No breaking changes in commit list - Analyzed",
            "Title",
            counter,
            #new_commit_list,
            true
         ...
         print_progress_percentage("Analyzing commits... Done", "String", counter, #new_commit_list, true)
      end
   ...
   local function reset_to_remote_head()
      ...
      local reset_status = utils.cmd("git -C " .. config_path .. " reset --hard origin/" .. update_branch, true)
      ...
      -- clean up the repo
      local clean_status = utils.cmd("git -C " .. config_path .. " clean -f -d", true)
   ...
   local function check_for_leftover_tmp_commit()
      ...
      if last_commit_message:match "^tmp$" then
         echo {
            {
               "Removing tmp commit. This has not been removed properly after the last "
                  .. "update. Cleaning up...\n\n",
               "WarningMsg",
            },
         }
         ...
         if not result:match "No local changes to save" then
            echo {
               {
                  "Local changes outside of the custom directory detected. They have be stashed "
                     .. 'using "git stash"!\n\n',
                  "WarningMsg",
               },
            }
   ...
   if continue == nil and skip_confirmation == nil then
      ...
      utils.clear_last_echo()
      echo {
         { "NvChad's HEAD has successfully been reset to ", "Title" },
         { update_branch },
         { ".\n\n", "Title" },
      }
