...
M.already_on_snapshot_branch = { { "You are already on a snapshot branch (", "WarningMsg" }, { "<SNAP_NAME>" },
   { "). You cannot create a snapshot of a snapshot.", "WarningMsg" } }
M.already_on_selected_snapshot_branch = { { "You are already on the selected snapshot branch (", "Title" },
   { "<SNAP_NAME>" }, { ").", "Title" } }
M.already_on_selected_update_branch = { { "You are already on the selected update branch (", "Title" },
   { "<SNAP_NAME>" }, { ").", "Title" } }
...
M.applying_git_stash_failed = { { "Applying git stash to" }, { "<BRANCH_NAME>" },
   { " failed! Remove all merge conflicts to apply the stash.", "WarningMsg" } }
M.branch_already_exists = { { "A Snapshot with the name ", "WarningMsg" }, { "<BRANCH_NAME>" },
   { " already exists! Would you like to [o]verwirte this snapshot, use another [<name>] or [a]bort? [<name>/o/A]",
      "WarningMsg" } }
...
M.checkout_failed = { { "\nChecking out branch ", "WarningMsg" }, { "<NEW_BRANCH_NAME>" }, { " failed!", "WarningMsg" },
   { " Current branch is: ", "WarningMsg" }, { "<OLD_BRANCH_NAME>" }, { ".", "WarningMsg" } }
...
M.create_branch_failed = { { "Error: Could not create branch ", "WarningMsg" }, { "<BRANCH_NAME>" },
   { "!", "WarningMsg" } }
...
M.modifications_detected = { { "Warning\n  Modification to repo files detected.\n\n  Updater will run", "WarningMsg" },
   { " git reset --hard " }, { "in config folder, so changes to existing repo files except ", "WarningMsg" },
   { "lua/custom folder" }, { " will be lost!\n", "WarningMsg" } }
M.modifications_detected_stash = { { "Local changes outside of the custom directory detected. " ..
    'They have been stashed with "git stash"!\n', "WarningMsg", } }
...
M.new_commits_summary = { { "There ", "Title" }, { "<HR_HAVE>", "Title" }, { " been", "Title" },
   { " <HR_NEW_COMMIT_LIST> " }, { "new ", "Title" }, { "<HR_COMMITS>", "Title" },
...
M.not_a_git_dir = { { "Error: ", "ErrorMsg" }, { "<CONFIG_PATH>", "ErrorMsg" },
   { " is not a valid git directory.\n", "ErrorMsg" } }
M.no_snapshots_found = { { "No snapshots found! Use ", "WarningMsg" }, { "NvChadSnapshotCreate" },
   { " to create your first snapshot.", "WarningMsg" } }
M.no_snapshots_selected = { { "No snapshots selected!" } }
M.packer_sync = { { "Would you like to run ", "WarningMsg" }, { "PackerSync" },
   { " after the update has completed?\n", "WarningMsg" }, { "Not running ", "WarningMsg" }, { "PackerSync" },
   { " may break NvChad! ", "WarningMsg" }, { "[y/N]", "WarningMsg" } }
...
M.removing_tmp_commit = { { "Removing tmp commit. This has not been removed properly after the " ..
    "last update. Cleaning up...\n\n", "WarningMsg", } }
...
M.update_cancelled_up_to_date = { { "You are already up to date with ", "String" }, { "<UPDATE_BRANCH>" },
...
M.select_snapshot_to_checkout = { { "Select a snapshot that you would like to checkout or switch back to the   update branch:\n",
   "WarningMsg" } }
M.select_snapshot_to_checkout_enter_index = { { "\nEnter the index of the snapshot that you would like to checkout or [c]ancel [<number>/C]:",
   "WarningMsg" } }
M.select_snapshot_to_delete = { { "Select the snapshot(s) that you would like to delete:\n", "WarningMsg" } }
M.select_snapshot_to_delete_enter_index = { { "\nEnter the indices of the snapshots separated by space or [c]ancel [<number_list>/C]:",
   "WarningMsg" } }
M.snapshot_successfully_created = { { "Snapshot ", "Title" }, { "<SNAP_NAME>" },
   { " has been created successfully!", "Title" } }
...
M.snapshot_stay_or_return = { { "Would you like to [u]se the snapshot branch (", "WarningMsg" }, { "<BRANCH_NAME>" },
   { ") or [r]eturn to the update branch (", "WarningMsg" }, { "<UPDATE_BRANCH>" }, { ")? [u/R]", "WarningMsg" } }
